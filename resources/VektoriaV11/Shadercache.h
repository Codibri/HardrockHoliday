...
namespace Vektoria
{
	...
	public:
		...
		enum ECreateType
		{
			eCreateTypeForceCompile,	// Leere den Cache und kompiliere alle Shader neu
			eCreateTypeCompileChanges,  // Kompiliere die Shader, die sich geändert haben
			eCreateTypeUseCached	    // Benutze nur die Shader aus dem Cache		
		};

		// Automatisches Shader Rekompilieren
		enum EShaderAutoRecompileType
		{
			eShaderAutoRecompileDisabled, // Shader nur beim Programmstart kompilieren
			eShaderAutoRecompileEnabled   // Automatisches rekompilieren von Shadern
		};
		...
		CShaderCache(ID3D11Device* pDevice, ID3D11DeviceContext* pContext,
					 const EShaderAutoRecompileType eAutoRecompileTouchedShadersType = eShaderAutoRecompileDisabled,
		...
		HRESULT GenerateShaders(ECreateType createType, const bool bRecreateShaders = false);
