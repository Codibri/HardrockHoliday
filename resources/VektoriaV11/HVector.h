...
//---------------------------------------------------
// CHVector: Klasse f¸r homogene Vektoren
//
// Autoren: Prof. Dr. Tobias Breiner
// Letzte ƒnderung: 2012-06-05
//---------------------------------------------------
...
public:
	...
	CHVector(float fx, float fy, float fz, float fw); // Konstruktor 2 (alle Werte werden dediziert ¸bergeben)
	...
	CHVector operator +(void); // Positivzeichen vor Vektor => keine Ver‰nderung, trotzdem der mathematisch vollwertigenNotation halber wichtig
 
	CHVector operator^(CHVector & hvector); // Kreuzprodukt

	void Copy(CHVector & hvector);	// kopiert den Vektor

	void Init(float fx, float fy, float fz, float fw); // initialisiert oder ¸berschreibt die Werte des Vektors
	...
	CHVector Max(CHVector & hvector); // erzeugt Vektor aus den jeweils grˆﬂeren Tupeln
	...
	float Length();	// gibt Betrag (=L‰nge) des Vektors als Skalar aus
	...
	float Angle(CHVector & v); // gibt Winkel im Bogenmaﬂ aus, der sich zwischen dem bestehenden Vektor und dem Vector v befindet.
	float AngleXY(); // gibt auf die XY-Ebene projizierten Drehwinkel im Bogenmaﬂ aus (Null entspricht X-Achse);
	float AngleZX(); // gibt auf die XZ-Ebene projizierten Drehwinkel im Bogenmaﬂ aus (Null entspricht Z-Achse);
	float AngleYZ(); // gibt auf die YZ-Ebene projizierten Drehwinkel im Bogenmaﬂ aus (Null entspricht Y-Achse);

	float AngleYX(); // gibt auf die XY-Ebene projizierten Drehwinkel im Bogenmaﬂ aus (Null entspricht Z-Achse);
	float AngleXZ(); // gibt auf die XZ-Ebene projizierten Drehwinkel im Bogenmaﬂ aus (Null entspricht X-Achse);
	float AngleZY(); // gibt auf die YZ-Ebene projizierten Drehwinkel im Bogenmaﬂ aus (Null entspricht Z-Achse);
