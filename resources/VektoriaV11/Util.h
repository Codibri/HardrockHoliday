...
//---------------------------------------------------
// Util.h: Datei für einfache Hilfsfunktionen
//
// Autor: Matthias Sommer & Tobias Breiner
// Letzte Änderung: 2011-12-02
//---------------------------------------------------
// Zahlenkonvertierung:
#define UM_HZ2SEC(x) (1/x) // converts hertz to seconds
#define UM_RAD2DEG(x) (x*180.0/M_PI) // converts radians to degrees, M_PI is the value of pi and is defined in math.h library
#define UM_DEG2RAD(x) (x*M_PI/180.0) // converts degrees to radians 
...
#define UM_ISINRANGE(f,fMin,fMax) ((f>=fMin)&&(f<=fMax))  // Überprüft, ob Wert f in den Wertebereich [fMin bis fMax] liegt, wenn ja => true, falls nein => false
...
// Zufallsmakros die rand() aus time.h verwenden (es muss vor dem Verweden dieser Makros srand() aufgerufen werden):
